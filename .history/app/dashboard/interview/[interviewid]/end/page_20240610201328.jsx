...
const Feedback = ({ params }) => {
  ...
  return (
    <div className='p-5 md:p-10'>
          ...
          <Collapsible key={index} className="border-b border-gray-200 mb-4">
            ...
            <CollapsibleContent className="p-4 bg-white rounded-b-md">
              <div className='space-y-2'>
                <p className='bg-yellow-100 w-full p-4 rounded-t-md' ><strong className='text-yellow-400'>Rating:</strong>  {feedback.rating}</p>
                <p className='bg-red-100 w-full p-4 rounded-t-md' ><strong className='text-red-400'>Your Answer:</strong> {feedback.useranswer}</p>
                <p className='bg-green-100 w-full p-4 rounded-t-md'><strong className='text-green-400'>Correct Answer:</strong> {feedback.correctAns}</p>
                <p className='bg-blue-100 w-full p-4 rounded-t-md'><strong className='text-blue-400'>Feedback:</strong> {feedback.feedback}</p>
              </div>
            </CollapsibleContent>
          </Collapsible>
        ))}
      ...
      <Button colorScheme='blue' onClick={()=>router.replace('/dashboard')} >Go Home</Button>
    </div>
  )
}
