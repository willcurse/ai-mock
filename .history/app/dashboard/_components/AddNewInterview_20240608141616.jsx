...
const AddNewInterview = () => {
  ...
  const onSubmit = async (e) => {
    ...
    console.log(JSON.parse(MockToJson));
    setJsonSave(MockToJson);

    if (MockToJson) {
      // Ensure numeric yearsOfExperience before insertion
      const numericExp = parseInt(yearsOfExperience, 10);

      try {
        const resp = await db.insert(mySchemaUsers)
          .values({
            Mockid: uuidv4(),
            jsonMockResp: MockToJson,
            jobposition: jobPosition,
            jobdesc: jobDescription,
            jobexp: numericExp, // Use parsed numeric value
        ...
        // Handle database insert error (e.g., display message to user)
      }
    } else {
      console.log("Error");
    }
